...
export default function Header01() {
  ...
  const getBalance = async () => {
    ...
    // get the balance of the current account in wei
    const balanceInWei = await web3.eth.getBalance(currentAccount);

    // convert the balance from wei to ether
    const balanceInEther = web3.utils.fromWei(balanceInWei, "ether");
    setBalance(balanceInEther.slice(0, 5));
  };
                  ...
                  <Link href="/myorders" legacyBehavior>
                    <a>
                      <button className="text-jacarta-700 font-display hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5">
                        <span
                          className={
                            isChildrenPageActive(route.asPath, "/myorders")
                              ? "text-accent dark:text-accent"
                              : ""
                          }
                        >
              ...
              <Link href="/explore" legacyBehavior>
                <a>
                  <button className="text-jacarta-700 font-display hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5">
                    <span
                      className={
                        isChildrenPageActive("/explore", route.asPath)
                          ? "text-accent dark:text-accent"
                          : ""
                      }
                    >
              ...
              <Link href="/myorders" legacyBehavior>
                <a>
                  <button className="text-jacarta-700 font-display hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5">
                    <span
                      className={
                        isChildrenPageActive("/myorders", route.asPath)
                          ? "text-accent dark:text-accent"
                          : ""
                      }
                    >
