...
import { setConnection, walletModalShow } from "../../redux/counterSlice";
...
export default function WalletButton() {
  ...
  useEffect(() => {
    if (status === "connected") dispatch(setConnection());
  }, [status]);
  if (status === "initializing") return <div>Ongoing...</div>;

  if (status === "unavailable") return <div>Metamask not detected</div>;
